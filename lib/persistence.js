...
function Persistence (options) {
  ...
  // select the proper EOL sequence
...
Persistence.prototype.persistCachedDatabase = function (cb) {
  ...
  this.db.getAllData().forEach(function (doc) {
    toPersist += model.serialize(doc) + self.EOL;
  });
  Object.keys(this.db.indexes).forEach(function (fieldName) {
    if (fieldName != "_id") {   // The special _id index is managed by datastore.js, the others need to be persisted
      toPersist += model.serialize({ $$indexCreated: { fieldName: fieldName, unique: self.db.indexes[fieldName].unique, sparse: self.db.indexes[fieldName].sparse }}) + self.EOL;
    }
  });
 ...
 */
Persistence.treatRawData = function (rawData, self) {
  var data = rawData.split(self.EOL);
  var dataById = {}
    , tdata = []
    , i
    , indexes = {}
      ...
      Persistence.ensureDirectoryExists(path.dirname(self.filename), function (err) {
        self.ensureDatafileIntegrity(function (exists) {
          fs.readFile(self.filename, 'utf8', function (err, rawData) {
            ...
            var treatedData = Persistence.treatRawData(rawData, self);
